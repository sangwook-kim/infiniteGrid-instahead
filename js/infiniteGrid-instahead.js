...
eg.module("infiniteGridInstaHead", ["jQuery", eg, window, document], function($, ns, global, doc) {
	"use strict";
	...
	ns.InfiniteGridInstaHead = ns.Class.extend(ns.Component, {
		_events: function() {
			return EVENTS;
		},
		construct: function(el, options, _prefix) {
			...
			this._onWheel = $.proxy(this._onWheel, this);
			this.$view.on("scroll", this._onScroll)
				.on("resize", this._onResize);

            this.$view.on('wheel', this._onWheel);
		},
        _onWheel: function() {
            var i = 0, heads = $('header').not(this.options.headHolder.find('header'));

            for(var l = heads.length;i < l; ++i) {
                if(heads[i].getBoundingClientRect().top < 1) {
                    if( $(heads[i]).hasClass('real_header') ) {
                        this.options.headHolder.empty().append($(heads[i]).clone());
                    }
                } else {
                    break;
                }
            ...
            if(i<1) {
                this.options.headHolder.empty();
                this.options.headHolder.css({padding: ''});
            } else if (i % 2 === 0) {
                this.options.headHolder.empty();
                this.options.headHolder.css({padding: ''});
                $(heads[i - 1]).css({visibility: 'visible'});
            } else {
                $(heads[i]).css({visibility: 'hidden'});
            }
        },
